...
test('numerals', () => {
	for (let i = -9999; i <= 9999; i++) {
		expect(fromDigitsLinzklar(toDigitsLinzklar(i))).toBe(i);
	}
});

test("incorrect numerals", () => {
	expect(() => fromDigitsLinzklar(["無", "無"])).toThrow();
	expect(() => fromDigitsLinzklar(["三", "無"])).toThrow();
	expect(() => fromDigitsLinzklar(["十", "下"])).toThrow();
});